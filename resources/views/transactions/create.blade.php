        ...
        <form action="{{ route('transactions.store') }}" method="POST" class="bg-white shadow rounded-lg p-6">
            ...
            <div class="mb-4">
                ...
                <textarea id="desc" name="desc" rows="2"
                    class="block w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('desc') }}</textarea>
                @error('desc')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-2">
                ...
                <button type="submit"
                    class="px-4 py-2 bg-indigo-600 rounded-md text-white hover:bg-indigo-700">Add</button>
            </div>
        </form>
    </div>
</x-app-layout>
